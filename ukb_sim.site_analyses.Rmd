...
```{r}
ggplot(cent.diffs2) +
  geom_histogram(aes(x=diff_rh_CT_posteriorcingulate, fill=dataset), alpha = 0.6)
```
...
for (pheno in pheno_diff.list) {
  ...
  df.pairwise.t <- tidy(pairwise.wilcox.test(x=cent.diffs2[[pheno]], g=cent.diffs2[["dataset"]], paired = TRUE, p.adj = "none")) #update to wilcox since t assumptions not all met
...
t tests of sex-biases in centiles w/in each feature **ask andrew about this**
