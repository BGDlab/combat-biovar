...
```{r}
#load data csvs
ukb_df_cf.gam <- fread(file="data/ukb_to_model/cf.gam_data.csv")
ukb_df_cf.gamlss <- fread(file="data/ukb_to_model/cf.gamlss_data.csv")
# length(ukb_df_cf.gamlss$participant) == n_unique(ukb_df_cf.gamlss$participant)
ukb_df_cf.lm <- fread(file="data/ukb_to_model/cf.lm_data.csv")
```

plot data distributions (not loading models for now)
```{r}
ven_raw <- ggplot(ukb_df_age.filt) +
  geom_point(aes(x=log_age, y=Ventricles, color=sex), alpha =0.3) +
  ggtitle("Raw Data")

ven_lm <- ggplot(ukb_df_cf.lm) +
  geom_point(aes(x=log_age, y=Ventricles, color=sex), alpha =0.3) +
  ggtitle("ComBat")

ven_gam <- ggplot(ukb_df_cf.gam) +
  geom_point(aes(x=log_age, y=Ventricles, color=sex), alpha =0.3) +
  ggtitle("ComBat-GAM")

ven_gamlss <- ggplot(ukb_df_cf.gamlss) +
  geom_point(aes(x=log_age, y=Ventricles, color=sex), alpha =0.3) +
  ggtitle("ComBatLS")

ggarrange(ven_raw, ven_lm, ven_gam, ven_gamlss, ncol=2, nrow=2, common.legend=TRUE)
```

```{r}
rh_CT_bankssts_raw <- ggplot(ukb_df_age.filt) +
  geom_point(aes(x=log_age, y=rh_CT_bankssts, color=sex), alpha =0.3) +
  ggtitle("Raw Data")

rh_CT_bankssts_lm <- ggplot(ukb_df_cf.lm) +
  geom_point(aes(x=log_age, y=rh_CT_bankssts, color=sex), alpha =0.3) +
  ggtitle("ComBat")

rh_CT_bankssts_gam <- ggplot(ukb_df_cf.gam) +
  geom_point(aes(x=log_age, y=rh_CT_bankssts, color=sex), alpha =0.3) +
  ggtitle("ComBat-GAM")

rh_CT_bankssts_gamlss <- ggplot(ukb_df_cf.gamlss) +
  geom_point(aes(x=log_age, y=rh_CT_bankssts, color=sex), alpha =0.3) +
  ggtitle("ComBatLS")

ggarrange(rh_CT_bankssts_raw, rh_CT_bankssts_lm, rh_CT_bankssts_gam, rh_CT_bankssts_gamlss, ncol=2, nrow=2, common.legend=TRUE)
```
...
for (pheno in pheno_diff.list) {
  ...
  #group-wise
  df.aov <- tidy(kruskal.test(cent.diffs2[[pheno]] ~ cent.diffs2[["dataset"]])) #nonparametric b/c distribution but also data is paired? ask andrews
  df.aov$pheno <- pheno
...
Visualize simulated sites (`scp -r 'gardnerm@cubic-login.uphs.upenn.edu:/cbica/home/gardnerm/combat-biovar/data/ukb_ratios/*.csv' ./data/ukb_ratios/`)


```{r}
ratio_data_path <- "/Users/megardn/Desktop/BGD_Repos/combat_biovar/data/ukb_ratios"
raw_files <- list.files(path = ratio_data_path, pattern = "_raw.csv", full.names = TRUE)
...
for (file in raw_files) {
  # Read each CSV file
    ...
    # Add a "Source_File" column with the file name
    data <- data %>%
      dplyr::select(c(sim.site, participant, sex, site, age_days)) %>%
      mutate(Source_File = as.factor(basename(file))) %>%
      mutate(dataset = gsub("_data|_raw.csv|prop-|[0-9]|-", "", Source_File))

    # Bind the data to the combined dataframe
    df <- bind_rows(df, data, .id = "File_ID")
    assign("ratio_sites", df)
}
length(unique(ratio_sites$Source_File))
```
...
```{r}
ggplot(ratio_sites) +
  geom_histogram(aes(x=age_days, fill=sim.site), position = "identity", alpha=0.4, binwidth = 365) +
  facet_wrap(~Source_File)

ggplot(ratio_sites) +
  geom_histogram(aes(x=age_days, fill=Source_File, color=Source_File), position = "identity", alpha=0.4, binwidth = 365)
```

```{r}
#path to csvs scp'd from cubic
ratio_path <- "/Users/megardn/Desktop/BGD_Repos/combat_biovar/derivatives/ukb_ratio"
```
...
```{r}
n_ratio_list <- lapply(c(1:11), str_pad, 2, pad = "0")
n_ratio_list <- paste0("prop-", n_ratio_list)
pheno_list.z <- paste0(pheno_list, ".z")

# combined_preds.p <- data.frame()
get.predictions.ratio <- function(x){
  df <- data.frame() #new empty dataframe
...
ratio_predictions <- mclapply(n_ratio_list, get.predictions.ratio, mc.preschedule = FALSE)
```
