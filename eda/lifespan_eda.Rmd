...
```{r}
#sample size, sex
length(unique(cn.df_mad_filt$participant))
nrow(cn.df_mad_filt) == length(unique(cn.df_mad_filt$participant))
...
```{r}
full_rain <- final_df %>%
  mutate(continent = factor(case_when(country == "India" | country == "China" | country == "Singapore" | country == "Japan" ~ "Asia",
                               country == "USA" | country == "Canada" ~ "North America",
                               country == "Brazil" ~ "South America",
                               country == "Australia" ~ "Australia",
                               TRUE ~ "Europe"))) %>%
  ggplot(aes(x=continent, y=age_years, fill=sex)) +
  geom_rain(alpha=.5, violin.args = list(scale="count", alpha=.5),
