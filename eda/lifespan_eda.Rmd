...
```
Write out data for andrew
```{r, eval=FALSE}
andrew.df <- cn.df.unique %>%
  dplyr::filter(study != "UKB" & age_days >= (365.25*18))
nrow(andrew.df)
fwrite(andrew.df, file=paste0(base_path, "/data/lifespan_CN_adult.csv"))
```
...
#   ggplot() +
#   geom_boxplot(aes(x=study, y=surfholes)) +
#   geom_point(aes(x=study, y=(study_euler_median+2*study_euler_madconst1)), color="red")
```
...
```{r}
cn.df.site <- cn.df.eulerfilt2 %>% 
  dplyr::filter(site != "NA") %>%
  mutate(study = droplevels(study),
         site = gsub(" ", "_", trimws(site))) %>%
  unite(site, study, site, sep = "_", remove = FALSE) %>% #so sites with the same name don't get combined across studies!
  na.omit()
...
```{r}
ggplot(cn.df.site) +
  geom_boxplot(aes(x=reorder(study, age_days), y=age_days))+
...
ggplot(cn.df.site) +
  geom_bar(aes(x=site, fill=sex), position="dodge") + 
...
```{r}
fwrite(cn.df.site, file=paste0(base_path, "/data/lifespan_CN_site-level_euler.csv"))
```
write out imagen data for testing
```{r}
imagen.df <- cn.df.site %>%
  dplyr::filter(study=="IMAGEN")
# skim(imagen.df)

fwrite(imagen.df, file=paste0(base_path, "/data/imagen_CN.csv"))
```

troubleshooting combat

```{r, eval=FALSE}
gamlss(formula = rh_Vol_posteriorcingulate ~ pb(age_days) + sexMale + pb(sex.age) + site, sigma.formula= ~ pb(age_days) + sexMale + site, data=imagen.df)
```
...
having some issues with combat on cubic, trying locally.
