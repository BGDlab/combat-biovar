...
# cf.lm_refA$dat.combat[, "sim.site"] <- batch
```
...
```{r}
cf.gam_refA <- comfam(pheno.df, batch, covar.mat, gam, formula = y ~ s(age_days) + sexMale, ref.batch = "Site_A")
# cf.gam_refA$dat.combat[, "sim.site"] <- batch
```
...
cf.gamlss_refA <- comfam(pheno.df, batch, covar.mat, gamlss, y ~ age_days + sexMale,
              ...
              control = gamlss.control(trace = FALSE),
              ref.batch = "Site_A")
# cf.gamlss_refA$dat.combat[, "sim.site"] <- batch
```
