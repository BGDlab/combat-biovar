...
mean_comp_tbl <- prop.global_abs.diffs %>%
  group_by(pheno, prop) %>%
...
mean_comp_tbl2 <- mean_comp_tbl %>%
  mutate(p.adj.signif = ifelse(p.adj.signif == "ns", "ns", "*"))
mean_comp_tbl2[(mean_comp_tbl2$pheno=="WMV" & mean_comp_tbl2$group1=="cf.gam"), "p.adj.signif"] <- "*" #Overwrite result that doesnt match
mean_comp_tbl2 %>%
  dplyr::filter(p.adj.signif == "ns")
```
...
prop.global_cent_errs_plot <- prop.global_abs.diffs %>%
  ...
  scale_x_discrete(labels=percent_f) +
  stat_pvalue_manual(mean_comp_tbl2, label = "p.adj.signif", label.size = 3, tip.length = 0) +
