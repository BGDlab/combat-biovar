...
unique(ratio.merge.df$dataset)
dim(ratio_raw_pred.df)
dim(ratio.merge.df)

ratio.merge.df <- ratio.merge.df %>%
  dplyr::select(!File_ID) %>%
  mutate(dataset=gsub("_predictions.csv","", dataset))
```
...
```{r}
# df_list <- c()
# for (p in unique(ratio.merge.df$prop)) {
#   df <- ratio.merge.df %>%
#     dplyr::filter(prop == p)
#   df_list[p] <- list(df)
# }
# length(df_list) == length(unique(ratio.merge.df$prop))
```

define function to remove extremes
```{r}
rm_ext_cents <- function(og.data, feat_list){
  df_ext_list <- c()
  count <- 1
  ...
  stopifnot(length(df_ext_list) == length(pheno_list))
...
```{r}
ratio_no_ext_list <- mclapply(df_list, rm_ext_cents, feat_list=pheno_list, mc.preschedule = FALSE)
```
