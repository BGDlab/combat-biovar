...
visualize magnitude of sex-biases
...
Look at differences in M:F for subjects w very high or very low centiles
```{r}
feat_exts_long <- ratio.df %>%
  ...
  pivot_longer(cols=c(pheno_list), names_to="pheno", values_to="centile_cat") %>%
   mutate(pheno_cat = as.factor(case_when(
    ...
    pheno %in% ct_list ~ "Regional CT",
    TRUE ~ NA_character_))) %>%
  mutate(centile_cat = case_when(. < 0.2 ~ "low",
                                        . > 0.8 ~ "high", 
                                        TRUE ~ "mid"))
feat_exts_long %>%
  dplyr::filter(centile_cat == "low") %>%
  ggplot() +
  geom_bar(aes(x=pheno_cat, fill=sex, color=sex)) +
  facet_grid(prop ~ dataset)

feat_exts_long %>%
  dplyr::filter(centile_cat == "high") %>%
  ggplot() +
  geom_bar(aes(x=pheno_cat, fill=sex, color=sex)) +
  facet_grid(prop ~ dataset)
```
