...
Look at differences in M:F for subjects w very high or very low mean centiles
```{r}
feat_exts <- ratio.df %>%
  dplyr::select(-contains(c("diff", ".z"))) %>%
  mutate(across(pheno_list, ~ case_when(. < 0.25 ~ "low",
                                        . > 0.75 ~ "high", 
                                        TRUE ~ "mid")))
  
feat_exts_long <- feat_exts %>%
  pivot_longer(cols=c(pheno_list), names_to="pheno", values_to="centile_cat") %>%
...
feat_exts_long %>%
  ggplot() +
  geom_col(aes(x=centile_cat, y=pheno_cat, fill=sex, color=sex)) +
  facet_grid(dataset ~ prop)
```

visualize actual (not ABSOLUTE) errors for global phenos by sex

```{r}
#trying to add stats test
...
look at just cf.gam & cf.gamlss

```{r, eval=FALSE}
cent.feat.sex.test.df %>%
  dplyr::filter(dataset == "cf.gamlss") %>%
  dplyr::select(pheno, prop, sig_fdr) %>%
  arrange(pheno, prop)
```
...
cent.feat.sex.test.df2 <- cent.feat.sex.test.df %>%
...
sex_t_tests_to_plot <- left_join(cent.feat.sex.test.df2, dk.parc, by="label")
...
z.subj.sex.test.df <- bind_rows(z.subj.sex.test.list)
```
...
```{r}
noext_z.subj.sex.test.list <- readRDS(paste0(data_path, "/no.ext_featurewise_z_sex_bias_tests.RDS"))
noext_z.subj.sex.test.df <- bind_rows(noext_z.subj.sex.test.list)
```

simple bar-plot to count regions w sigificant sex-biases

```{r}
noext_z.subj.sex.test.df %>%
  ggplot() +
  geom_bar(aes(x=prop, fill=sig_fdr)) +
  facet_wrap(~dataset) +
  scale_x_discrete(labels=percent_f) +
...
## FIGURE 4
```{r}
fig_4 <- ggarrange(ggarrange(sampling.plt, prop.global_cent_errs_plot, 
                             nrow = 2, labels = c("A", "C"),
...
fig_4_annotate <- annotate_figure(fig_4, top = text_grob("Figure 4. Varying M:F ratios across site simulations", color = "black", hjust = 0, x = 0))
fig_4_annotate
ggsave("figs/figure_4.jpeg", fig_4_annotate, width=33, height=20, units="cm")
```
