...
cent.t.test.df_global_filt <- cent.t.test_global %>%
...
stats_centile_for_plot <- inner_join(mean_comp_tbl2, cent.t.test.df_global_filt)
...
prop.global_cent_errs_plot <- prop.global_abs.diffs %>%
  ...
  stat_pvalue_manual(stats_centile_for_plot.corrected, label = "p.adj.signif", label.size = 2, tip.length = 0) +
...
cent.feat.test_global %>%
  dplyr::filter(pheno=="WMV") %>%
  dplyr::select(dataset, pheno, prop, sig_fdr)
```
...
sex_err_ratio.plt <- ratio.df %>%
  ...
  stat_pvalue_manual(stats_sex.centile_for_plot.corrected, label = "p.adj.signif", label.size = 2, tip.length = 0) +
...
t.test.cent.sex.diff <- cent.feat.sex.test.df %>%
  ...
  mutate(p.val_fdr = p.adjust(t.test.p, method="fdr", n = (3*4*11)), #not testing global vols, too few data points
         ...
         sig_fdr = case_when(p.val_fdr < 0.05 ~ "*",
                               p.val_fdr >= 0.05 ~ ""),
         y_pos = max_val + 0.1)
...
cortical_cent_sex_diff.plt <- cent.feat.sex.test.df %>%
  ...
  geom_boxplot(aes(color=dataset)) +
    facet_grid(dataset~pheno_cat, labeller=labeller(dataset=dataset_names), scales="free_y") + 
...
COME BACK TO THIS AFTER LOOKING AT REPLICATION PLOTS
...
## FIGURE 4
```{r}
fig_4 <- ggarrange(ggarrange(sampling.plt, sex_err_ratio.plt, 
                             nrow = 2, labels = c("A", "C"),
