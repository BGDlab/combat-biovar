...
```{r}
cent.t.test.df_global <- fread(paste0(der_path, "/ukb_ratio/full_featurewise_cent_t_tests_global.csv"))
...
global_sex_diffs <- cent.subj.sex.test.df %>%
...
```{r}
noext_cent.subj.sex.test.list <- readRDS(paste0(der_path, "/ukb_ratio/no.ext_featurewise_cent_sex_bias_tests.RDS"))
noext_cent.subj.sex.test.df <- bind_rows(noext_cent.subj.sex.test.list)
```

simple bar-plot to count regions w sigificant sex-biases

```{r}
noext_cent.subj.sex.test.df %>%
  ggplot() +
  geom_bar(aes(x=prop, fill=sig_fdr)) +
  facet_wrap(~dataset) +
  scale_x_discrete(labels=percent_f) +
...
```{r}
z.subj.sex.test.list <- readRDS(paste0(der_path, "/ukb_ratio/full_featurewise_z_sex_bias_tests.RDS"))
...
```{r}
noext_z.subj.sex.test.list <- readRDS(paste0(der_path, "/ukb_ratio/no.ext_featurewise_z_sex_bias_tests.RDS"))
