...
plot subj. with averages in extremes - **try also using averages w/in pheno categories**
```{r}
...
ext_cent.sum <- perm_subj.df2 %>%
  dplyr::filter(mean_centile < 0.25 | mean_centile > 0.75) %>%
  group_by(dataset, perm) %>%
  summarise(
    male_ratio = sum(sex == "Male") / n(),
    female_ratio = sum(sex == "Female") / n()
...
ext_cent.sum %>%
  ...
  geom_boxplot() +
  # geom_dotplot(binaxis = "y", stackdir="center")  +
  geom_hline(yintercept= ext_cent.sum[ext_cent.sum$dataset == "raw" & ext_cent.sum$perm=="perm-001",][["male_ratio"]], linetype="dashed")
...
too many males in high centiles, too few males in low centiles (combat-GAM trends opposite to this pattern)

overall - too few men relative to women in extreme centiles
