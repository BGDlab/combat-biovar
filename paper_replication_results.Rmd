...
```{r, warning=FALSE}
#apply
perm_t.tests <- mclapply(diffs_perm.list, centile.t.tests, mc.preschedule = FALSE, comp_multiplier=length(diffs_perm.list))
names(perm_t.tests) <- n_perm_list
cent_t.tests.p <- bind_rows(perm_t.tests, .id = "column_label")
...
```{r}
lapply(diffs_perm.list, plot.cent.diffs)
```
...
```{r, warning=FALSE}
sex_t_tests_in_feat.df <- mclapply(diffs_perm.list, sex.bias.feat.t.tests, mc.preschedule = FALSE, comp_multiplier=length(diffs_perm.list))
