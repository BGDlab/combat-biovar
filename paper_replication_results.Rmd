...
t.test.cent.sex.test.med <- cent.sex.test.med %>%
  ...
  mutate(p.val_fdr = p.adjust(t.test.p, method="fdr", n = nrow(cent.sex.test.med)),
...
#   dplyr::filter(VolReg_mean_centile > .85) %>% nrow()
...
high_cent.sum %>%
  ...
  facet_wrap(~pheno_cat, scales="free_y") +
  #geom_hline(yintercept=high_cent.sum[high_cent.sum$dataset == "raw" & high_cent.sum$perm=="perm-001",][["female_ratio"]], linetype="dashed")+
  labs(title="subjects with high average centiles")
...
low_cent.sum %>%
  ...
  facet_wrap(~pheno_cat, scales="free_y") +
  # geom_dotplot(binaxis = "y", stackdir="center")  +
  #geom_hline(yintercept= low_cent.sum[low_cent.sum$dataset == "raw" & low_cent.sum$perm=="perm-001",][["female_ratio"]], linetype="dashed") +
  labs(title="subjects with low average centiles")
...
ext_cent.sum %>%
  ...
  geom_boxplot() +
  # geom_dotplot(binaxis = "y", stackdir="center")  +
  #geom_hline(yintercept= ext_cent.sum[ext_cent.sum$dataset == "raw" & ext_cent.sum$perm=="perm-001",][["female_ratio"]], linetype="dashed")+
  facet_wrap(~pheno_cat, scales="free_y") +
  labs(title="subjects with extreme average centiles")
...
linear ComBat -> too few women with high centiles (avg. across phenos, pattern varies), too many women with low centiles; ~ balances out overall (except regional volume)

ComBat-GAM -> too many women with high centiles (small but pretty consistent pattern), too many women with low centiles (patter varies a bit across phenos); OVERALL TOO MANY WOMEN W EXTREME CENTILES 
...
ext_test <- lapply(cf_test_list, one_samp_wilcox, df=ext_cent.sum)
names(ext_test) <- cf_test_list
ext_test
```

by site
```{r}
perm_subj.df2.by.site <- perm_subj.df2 %>%
  group_by(perm, dataset, sim.site) %>%
  summarise(across(contains("mean"), list(site = mean)))

perm_subj.df2.by.site %>%
  ggplot() +
  geom_boxplot(aes(x=sim.site, y=mean_cent_abs.diff_site, fill=dataset)) +
  facet_wrap(~ dataset, scales="free")

perm_subj.df2.by.site %>%
    mutate(sim.site = factor(sim.site, levels=c("Site_B", "Site_A", "Site_C"), ordered=TRUE)) %>%
  ggplot() +
  geom_boxplot(aes(x=sim.site, y=mean_cent_abs.diff_site, fill=dataset)) +
  facet_wrap(~ dataset, scales="free")
...
perm_subj.df2.by.site %>%
  mutate(sim.site = factor(sim.site, levels=c("Site_B", "Site_A", "Site_C"), ordered=TRUE)) %>%
  ggplot() +
  geom_boxplot(aes(x=sim.site, y=mean_cent_diff_site, fill=dataset)) +
  facet_wrap(~ dataset, scales="free")
...
perm_subj.df2.by.site %>%
  ...
  mutate(sim.site = factor(sim.site, levels=c("Site_B", "Site_A", "Site_C"), ordered=TRUE)) %>%
  ggplot() +
  geom_boxplot(aes(x=sim.site, y=mean_centile_site, fill=dataset))

perm_subj.df2.by.site %>%
  dplyr::filter(dataset == "cf.gam" | dataset == "cf.gamlss") %>%
  pivot_wider(id_cols=c(perm, sim.site), names_from=dataset, values_from=mean_centile_site, names_prefix="site_mean_cent_") %>%
  mutate(delta_mean = site_mean_cent_cf.gam - site_mean_cent_cf.gamlss) %>%
  mutate(sim.site = factor(sim.site, levels=c("Site_B", "Site_A", "Site_C"), ordered=TRUE)) %>%
  ggplot() +
  geom_boxplot(aes(x=sim.site, y=delta_mean))
```
