...
cent.subj.sex.test.df <- cent.subj.sex.test.df %>%
...
cent.subj.sex.test.df %>%
  ggplot(aes(x=perm, fill=significant)) +
  geom_bar() +
  facet_wrap(~dataset) +
  theme(axis.text.x = element_blank()) +
  scale_fill_manual(values = c("#EA6F5F", "#F3AEA6", "#5FDAEA"))
```

removing fdr correction

```{r}
#get range
cent.subj.sex.test.df %>%
  mutate(median_sex_diff_fdr = case_when(sig_fdr == FALSE ~ NA,
  ...
  summarize(min_sex_diff = min(median_sex_diff, na.rm=TRUE),
            min_sex_diff_fdr = min(median_sex_diff_fdr, na.rm=TRUE),
            max_sex_diff = max(median_sex_diff, na.rm=TRUE),
            max_sex_diff_fdr = max(median_sex_diff_fdr, na.rm=TRUE))
...
t.test.cent.sex.test.med <- cent.sex.test.med %>%
  ...
  mutate(p.val_fdr = p.adjust(t.test.p, method="fdr", n = length(unique(cent.sex.test.med$dataset))*length(unique(cent.sex.test.med$pheno_cat))),
                  sig_fdr = case_when(p.val_fdr < 0.001 ~ "***",
                               ...
                               p.val_fdr >= 0.05 ~ ""),
         sig = case_when(t.test.p < 0.001 ~ "***",
                               t.test.p >= 0.001 & t.test.p < 0.01 ~ "**",
                               t.test.p >= 0.01 & t.test.p < 0.05 ~ "*",
                               t.test.p >= 0.05 ~ ""),
...
t.test.cent.sex.test.med_tbl <- t.test.cent.sex.test.med %>%
  dplyr::select(dataset, pheno_cat, t.test.est, t.test.stat, t.test.df, t.test.p, sig, sig_fdr) %>%
  ...
  rename("ComBat Method" = dataset,
         ...
         "p-value" = t.test.p,
         "Significance" = sig,
         "(FDR-corrected)" = sig_fdr) %>%
  gt() %>%
  tab_style(style = list(cell_fill(color = "gray95")),
    locations = cells_row_groups()) %>%
  tab_options(column_labels.font.weight = "bold") %>%
  tab_header(title = "One-sample t-tests of sex differences in centile errors across 100 replications")
...
t.test.cent.sex.test.med %>%
  ggplot() +
  geom_col(aes(x=pheno_cat, y=sig, fill=dataset), position="dodge")
...
sex_bias_density_plt <- cent.sex.test.med %>%
  ...
  theme_linedraw() +
  geom_text(aes(label = sig, x= x_pos*100, y=y_pos),
...
get_t_tests <- function(df, raw_df){
  ...
  out <- df %>%
    ...
    group_by(dataset) %>%
    summarise(pheno_cat = cat,
              max_val = max(female_ratio),
              mu=mu,
            t.test.p = tidy(t.test(female_ratio, mu=mu))$p.value,
            t.test.est = tidy(t.test(female_ratio, mu=mu))$estimate,
            t.test.stat = tidy(t.test(female_ratio, mu=mu))$statistic,
            t.test.df = tidy(t.test(female_ratio, mu=mu))$parameter,) %>%
    mutate(p.val_fdr = p.adjust(t.test.p, method="fdr", n = (4*4*2)), #adjusting for 4 methods * 4 pheno classes * 2 centile categories
           sig_fdr = case_when(p.val_fdr < 0.001 ~ "***",
                               p.val_fdr >= 0.001 & p.val_fdr < 0.01 ~ "**",
                               p.val_fdr >= 0.01 & p.val_fdr < 0.05 ~ "*",
                               p.val_fdr >= 0.05 ~ "NS"),
...
highlow_t_test.df_tbl <- all_t_test.df %>%
  dplyr::select(dataset, pheno_cat, source, t.test.est, t.test.stat, t.test.df, t.test.p, sig, sig_fdr) %>%
  mutate(dataset = case_when(dataset == "cf" ~ "ComBat w/o Covariates",
                                   dataset == "cf.lm" ~ "Linear ComBat",
                                   dataset == "cf.gam" ~ "ComBat-GAM",
                                   dataset == "cf.gamlss" ~ "ComBatLS"),
         source = ifelse(source=="high", ">80%", "<20%")) %>%
  rename("ComBat Method" = dataset,
         "Centile Group" = source,
         "Phenotype Category" = pheno_cat,
         "Estimate" = t.test.est,
         "t-statistic" = t.test.stat,
         "dfs" = t.test.df,
         "p-value" = t.test.p,
         "Significance" = sig,
         "Significance (FDR-corrected)" = sig_fdr) %>%
  gt() %>%
  tab_style(style = list(cell_fill(color = "gray95")),
    locations = cells_row_groups()) %>%
  tab_options(column_labels.font.weight = "bold") %>%
  tab_header(title = "One-sample t-test of female's over- or under-representation in high and low centile groups")
gtsave(highlow_t_test.df_tbl, filename="figs/cent_sexratios_highlow_table.png")
```
...
all_cent_sum.df %>%
  ...
  geom_text(aes(label = sig, y = max_val + 0.01),
              data = all_t_test.df) +
...
all_cent_sum_boxplt.diff <- all_cent_sum.df_scaled %>%
  ...
  geom_text(aes(label = sig, y = (y_pos)), nudge_y=.005,
...
all_cent_sum_boxplt.diff_full <- all_cent_sum.df_scaled %>%
  ...
  theme(legend.position="none", plot.title = element_text(size=12), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(y="Female Bias", x = "Dataset")
ggsave(filename="figs/all_cent_sum_boxplt_wtest.png", all_cent_sum_boxplt.diff_full, height=9, width=4.5)
```
...
ggplot() +
  ...
  theme(legend.position = "none") +
  labs(x="Male - Female Median Centile Errors", y="Number of Features") +
  ggtitle("Sex differences in median z-score errors across 100 sampling replications")
ggsave(filename="figs/z.sex_bias_hist_plt.z.png", z.sex_bias_hist_plt)
```
...
z.sex_bias_density_plt <- z.sex.test.med %>%
  ...
  facet_wrap(~pheno_cat) +
  theme_linedraw() +
  geom_text(aes(label = sig, x= x_pos, y=y_pos),
...
ggsave("figs/z_sex_boxplts.jpeg", z_sex_boxplts, height=8, width=5)
```
