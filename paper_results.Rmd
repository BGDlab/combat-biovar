...
modifying to test w/in feature rather than mean centiles

```{r}
sex.bias.feat.t.tests <- function(df, feature_list=pheno_list, comp_multiplier=1){
  ...
  df.sex.t <- df %>%
    group_by(dataset) %>%
    summarise(p.value = tidy(rank.welch.t.test.formula(formula=as.formula(paste0("diff_",pheno, " ~ sex")), paired = FALSE, p.adj = "none"))$p.value,
              sex_diff = tidy(rank.welch.t.test.formula(formula=as.formula(paste0("diff_",pheno, " ~ sex")), paired = FALSE, p.adj = "none"))$estimate,
              t.stat = tidy(rank.welch.t.test.formula(formula=as.formula(paste0("diff_",pheno, " ~ sex")), paired = FALSE, p.adj = "none"))$statistic,
              df = tidy(rank.welch.t.test.formula(formula=as.formula(paste0("diff_",pheno, " ~ sex")), paired = FALSE, p.adj = "none"))$parameter) %>%
    ungroup()
  df.sex.t$pheno <- pheno
...
trying to get interpretable effect sizes w t.test (rather than ranked t test)

```{r}
sex.bias.feat.t.tests_2 <- function(df, feature_list=pheno_list, comp_multiplier=1){
  ...
  df.sex.t <- df %>%
    group_by(dataset) %>%
    summarise(p.value = tidy(t.test(formula=as.formula(paste0("diff_",pheno, " ~ sex")), paired = FALSE, p.adj = "none"))$p.value,
              sex_diff = tidy(t.test(formula=as.formula(paste0("diff_",pheno, " ~ sex")), paired = FALSE, p.adj = "none"))$estimate,
              t.stat = tidy(t.test(formula=as.formula(paste0("diff_",pheno, " ~ sex")), paired = FALSE, p.adj = "none"))$statistic,
              df = tidy(t.test(formula=as.formula(paste0("diff_",pheno, " ~ sex")), paired = FALSE, p.adj = "none"))$parameter) %>%
    ungroup()
  df.sex.t$pheno <- pheno
  t.df <- rbind(t.df, df.sex.t)
  }
...
Foregoing anova based on emails with andrew.
...
```{r}
sex.bias.feat.t.tests(diffs.df)
```

Sig test w/in feature diffs
```{r, warning=FALSE}
sex_t_tests_in_feat.df <- sex.bias.feat.t.tests_2(diffs.df)
```

```{r}
sex_t_tests_in_feat.df.sum <- sex_t_tests_in_feat.df %>%
    group_by(dataset) %>%
    dplyr::summarize(n_sig_regions = sum(sig_fdr),
              min_pval = min(p.val_fdr),
              max_pval = max(p.val_fdr)) %>%
...
sex.var.plot.df <- sex_t_tests_to_plot %>%
  ...
  mutate(dataset = factor(dataset, levels = c("ukb_CN", "cf", "cf.lm", "cf.gam", "cf.gamlss"), ordered = TRUE),
         sex_diff = case_when(sig_fdr == FALSE ~ NA,
                              TRUE ~ sex_diff)) %>%
...
sex.var.plot <- ggplot(sex.var.plot.df) +
  ...
  paletteer::scale_fill_paletteer_c("grDevices::Cork", name = "Sex Effect", limits = c(-0.003, 0.003))
sex.var.plot
...
```{r}
perm_data_path <- "/Users/megardn/Desktop/BGD_Repos/combat_biovar/derivatives/ukb_perm"
...
pheno_abs.diff.list.z <- paste0("abs.diff_", pheno_list, ".z")
...
z.t.tests <- function(df){
  ...
  #run tests
  attach(df)
  for (pheno in pheno_abs.diff.list.z) {
