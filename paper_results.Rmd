...
pacman::p_load(data.table, ggplot2, tidyverse, ggseg, ggsegExtra, paletteer, ggpubr, gridExtra, lemon, parallel, rstatix, lme4, interactions)
...
Within each combat config, test if there are significant differences in M and F subject's mean abs. centile error. Can test mean (not abs) centile errors, Z errors, etc, using `to_test` arg. FDR corrects for number of 
...
```{r}
qc_cent_df <- combined_preds %>% 
  filter_at(vars(pheno_list), any_vars(. %in% c(1, 0, Inf, -Inf, NA))) %>%
  dplyr::select(File_ID, participant, sex, dataset, pheno_list) %>%
  pivot_longer(cols=pheno_list, names_to="pheno", values_to="centile")
skimr::skim(qc_cent_df)

qc_cent_df %>%
  dplyr::filter(centile == 0)

qc_cent1_df <- qc_cent_df %>%
  dplyr::filter(centile == 1) 

qc_cent1_df %>%
  ggplot() +
  geom_bar(aes(dataset, fill=participant)) +
  facet_wrap(~pheno)

ppt.list <- qc_cent1_df %>%
...
qc_z_df <- combined_preds %>% 
  filter_at(vars(pheno_list.z), any_vars(. %in% c(1, 0, Inf, -Inf, NA)))

unique(qc_z_df$participant)

qc_cent_df %>%
  dplyr::filter(centile != 1) %>%
  summarize(sprintf("%.100f", (1 - max(centile))))

qc_cent_df %>%
  summarize(sprintf("%.100f", min(centile)))
```
...
z_abs.diffs <- combined_preds %>%
  ...
  ungroup() %>%
  dplyr::filter(dataset != "ukb_CN") #drop raw
saveRDS(z_abs.diffs, "data/z_abs.diffs.rds")
```

```{r}
z_abs.diffs <- readRDS("data/z_abs.diffs.rds")
```
...
global.cent.df %>%
  group_by(pheno) %>%
  summarise(max=max(centile_5))

names(global.cent.df)


gmv.df <- global.cent.df

gmv.df <- centiles.df %>%
  dplyr::filter(pheno =="GMV")

  ggplot(gmv.df) +
      #geom_point(aes(x=ukb_cn_df$age_days, y=ukb_cn_df$GMV), alpha=0.5) +
      #scale_colour_manual(values=c("#5AAE61FF", "#9970ABFF")) +
      geom_line(aes(x=gmv.df$age_days, y=gmv.df$M_centile_1, color=gmv.df$dataset)) +
      geom_line(aes(x=gmv.df$age_days, y=gmv.df$M_centile_2, color=gmv.df$dataset)) +
      geom_line(aes(x=gmv.df$age_days, y=gmv.df$M_centile_3, color=gmv.df$dataset)) +
      geom_line(aes(x=gmv.df$age_days, y=gmv.df$M_centile_4, color=gmv.df$dataset)) +
      geom_line(aes(x=gmv.df$age_days, y=gmv.df$M_centile_5, color=gmv.df$dataset)) +
      facet_wrap(~ pheno, scales="free_y") + 
      labs(title="Male GMV brain charts")
  
      geom_line(aes(x=ages, y=pred$fanCentiles[[2]])) +
