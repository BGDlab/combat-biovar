...
```{r}
raw_files <- list.files(path = paste0(data_path, "/ukb_perm"), pattern = "_diffs.csv", full.names = TRUE)
...
```{r}
raw_files <- list.files(path = paste0(data_path, "/ukb_perm"), pattern = "_diffs.csv", full.names = TRUE) %>% unlist()
...
for (file in raw_files) {
  # Read each CSV file
    ...
    assign("diffs_subj.list", df_list)
}
length(diffs_subj.list)
diffs_subj.df <- bind_rows(diffs_subj.list)
```

Read in subject-wise summary stats

```{r}
raw_files <- list.files(path = paste0(data_path, "/ukb_perm"), pattern = "_subj_pred.csv", full.names = TRUE)
...
Do each feature's centile scores change based on combat version?

```{r}
#apply
perm_t.tests <- mclapply(diffs_subj.list, centile.t.tests, mc.preschedule = FALSE, comp_multiplier=length(diffs_subj.list))
names(perm_t.tests) <- n_perm_list
cent_t.tests.p <- bind_rows(perm_t.tests, .id = "column_label")
fwrite(cent_t.tests.p, file="derivatives/ukb_perm/centile_t.tests.csv")
```
...
```{r}
lapply(diffs_subj.list, plot.cent.diffs)
```
...
cent.diffs.v.plot <- perm_subj.df %>%
  ...
  ggtitle("Fig 1B. Within-subject mean magnitude of error in centile scores across ComBat \nconfigurations replicated 10x")
cent.diffs.v.plot
