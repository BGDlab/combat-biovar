...
sex_t_tests_in_feat.df <- sex_t_tests_in_feat.df %>%
  mutate(sig_uncor = ifelse(p.value<0.05, TRUE, FALSE)) %>%
...
sex_t_tests_in_feat.df %>%
    group_by(dataset) %>%
    dplyr::summarize(n_sig = sum(sig_uncor),
      n_sig_regions_fdr = sum(sig_fdr),
              min_pval = min(p.value),
              max_pval = max(p.value)) %>%
    ungroup() %>%
    as.data.frame()
```
...
sex.var.plot.df <- sex_t_tests_to_plot %>%
  ...
  mutate(dataset = factor(dataset, levels = c("raw", "cf", "cf.lm", "cf.gam", "cf.gamlss"), ordered = TRUE),
         median_sex_diff = ifelse(sig_uncor == TRUE, median_sex_diff*100, NA))
#find range
...
noext.sex_t_tests_in_feat.df <- noext.sex_t_tests_in_feat.df %>%
  mutate(sig_uncor = ifelse(p.value<0.05, TRUE, FALSE)) %>%
  mutate(significant = case_when(sig_fdr == TRUE ~ "Survives FDR Correction",
                                 sig_uncor == TRUE & sig_fdr == FALSE ~ "Uncorrected Only",
                                 sig_uncor == FALSE & sig_fdr == FALSE ~ "Not Significant")) %>%
...
noext.sex_t_tests_in_feat.df %>%
    group_by(dataset) %>%
    dplyr::summarize(n_sig_regions = sum(sig_uncor),
              min_pval = min(p.val_fdr),
              max_pval = max(p.val_fdr)) %>%
    ungroup() %>%
    as.data.frame()
...
sex.var.plot.full.df <- sex_t_tests_to_plot %>%
  dplyr::filter(!grepl('_refA', dataset)) %>% #drop refA levels
  mutate(dataset = factor(dataset, levels = c("raw", "cf", "cf.lm", "cf.gam", "cf.gamlss"), ordered = TRUE),
         median_sex_diff = case_when(sig_uncor == FALSE ~ NA,
                              TRUE ~ median_sex_diff*100)) %>%
...
z.sex_t_tests_in_feat.df <- z.sex_t_tests_in_feat.df %>%
  mutate(sig_uncor = ifelse(p.value<0.05, TRUE, FALSE)) %>%
  mutate(significant = case_when(sig_fdr == TRUE ~ "Survives FDR Correction",
                                 sig_uncor == TRUE & sig_fdr == FALSE ~ "Uncorrected Only",
                                 sig_uncor == FALSE & sig_fdr == FALSE ~ "Not Significant")) %>%
...
z.sex.var.plot.df <- z.sex_t_tests_to_plot %>%
  dplyr::filter(pheno_cat != "Global Volume" & dataset != "raw") %>%
  mutate(dataset = factor(dataset, levels = c("raw", "cf", "cf.lm", "cf.gam", "cf.gamlss"), ordered = TRUE),
         median_sex_diff = ifelse(sig_uncor == TRUE, median_sex_diff, NA))
...
noext.z.sex_t_tests_in_feat.df <- noext.z.sex_t_tests_in_feat.df %>%
  mutate(sig_uncor = ifelse(p.value<0.05, TRUE, FALSE)) %>%
  mutate(significant = case_when(sig_fdr == TRUE ~ "Survives FDR Correction",
                                 sig_uncor == TRUE & sig_fdr == FALSE ~ "Uncorrected Only",
                                 sig_uncor == FALSE & sig_fdr == FALSE ~ "Not Significant")) %>%
...
sex.var.plot.noext_z <- ggplot(sex.var.plot.df.noext_z) +
  ...
  facet_grid(dataset ~ pheno_cat, labeller = labeller(dataset = as_labeller(dataset_names))) + 
  theme(axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_rect(color= "gray", fill = NA), aspect.ratio=2/3, plot.margin = margin(1,1,1,1, 'pt')) +
  paletteer::scale_fill_paletteer_c("grDevices::Cork", name = "Male - Female Median \nZ-score Error", limits=c(-0.4, 0.4)) #
```
...
ven_plot <- ggplot(ven.df) +
  ...
  xlab("Age (Years Post-Conception)") + ylab("Volume (mm3)") +
...
sex_eff_comp_plt <- ggboxplot(raw_sex_eff, x = "pheno_cat", y = "sex_standard",
          color = "pheno_cat")+ 
