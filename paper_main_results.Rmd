...
1) is the slope nonzero in the full sample and within each site?

```{r}
diffs.df %>%
  dplyr::filter(dataset == "cf.gamlss" | dataset == "cf.gam" | dataset == "cf.lm") %>%
  ggplot(aes(x=rh_Vol_superiortemporal, y=diff_rh_Vol_superiortemporal)) +
...
subj_sum.df %>%
  dplyr::filter(dataset == "cf.gamlss" | dataset == "cf.gam" | dataset == "cf.lm") %>%
  ggplot(aes(x=mean_centile, y=mean_cent_diff)) +
  geom_point(alpha=0.3) +
  geom_smooth(method="lm") +
  facet_wrap(~dataset, scales="free_y")
...
2) is the slope of the nondominant sex in each site nonzero?


```{r}
diffs.df %>%
  dplyr::filter(dataset == "cf.gamlss" | dataset == "cf.gam" | dataset == "cf.lm") %>%
  ggplot(aes(x=rh_Vol_superiortemporal, y=diff_rh_Vol_superiortemporal, color=sex)) +
  geom_point(alpha=0.3) +
  geom_smooth(method="lm") +
  facet_wrap(~dataset, scales="free_y")
