...
```
Look at relative estimates for sex in sigma across features in raw data

```{r}
gamlss.sum.df <- fread(paste0(data_path, "/ukb_basic/gamlss_summary.csv"))
gamlss.sum.df.raw <- gamlss.sum.df %>%
  dplyr::filter(dataset=="ukb-CN-data-simsites")

gamlss.sum.df.raw.sexsig <- gamlss.sum.df.raw %>%
  dplyr::filter(parameter =="sigma" & term == "sexMale") %>%
  mutate(pheno_cat = as.factor(case_when(
      ...
      TRUE ~ NA_character_)),
      est_over_moment = estimate/moment.mean)

gamlss.sum.df.raw.sexsig %>%
  ggplot(aes(y=estimate, x=pheno_cat, fill=pheno_cat)) +
  geom_boxplot() +
  # geom_hline(yintercept = mean(gamlss.sum.df.raw.sexsig$t_stat), linetype = 2) +
  stat_compare_means(method = "anova", label.y = 0.07)+    
  stat_compare_means(label = "p.signif", method = "t.test",
                     ref.group = ".all.")

gamlss.sum.df.raw.sexsig %>%
  ggplot() +
  geom_histogram(aes(x=est_over_moment, fill=pheno_cat, color=pheno_cat), alpha=0.5, position="identity")

gamlss.sum.df.raw.sexsig %>%
  ggplot(aes(y=t_stat, x=pheno_cat, fill=pheno_cat)) +
  geom_boxplot() +
  # geom_hline(yintercept = mean(gamlss.sum.df.raw.sexsig$t_stat), linetype = 2) +
...
long_abs.diffs %>%
  ...
  ggplot(aes(pheno, abs.diff)) +
  geom_violin(aes(x=interaction(pheno,pheno_cat, sep = "&"), y=abs.diff,  fill=dataset, color=dataset), scale="count") +
  theme_linedraw(base_size = 11) +
  scale_fill_manual(values=c(cf.gam = "#3D75C2", cf.lm = "#75C23D", cf.gamlss ="#CFB023FF"), name = "ComBat \nConfiguration", labels=dataset_names) +
  scale_color_manual(values=c(cf.gam = "#3D75C2", cf.lm = "#75C23D", cf.gamlss ="#CFB023FF"), name = "ComBat \nConfiguration", labels=dataset_names) +
  labs(y="Absolulte Centile Error", x = "Feature") +
...
cfgam.better <- intersect(gam_better$pheno, ns_gam_gamlss_comp$pheno)
```
```{r, eval=FALSE}
cfgam.better.ct <- gsub("_CT", "", cfgam.better)
cfgam.better.ct_plot <- dk.parc %>%
  dplyr::mutate(gam_better = ifelse(label %in% cfgam.better.ct, 1, 0))

ggplot(cfgam.better.ct_plot) +
  theme_linedraw(base_size=12) +
  ggseg::geom_brain(atlas = dk_nomed, 
             position = position_brain(hemi ~ side),
            aes(fill=as.factor(gam_better)))
```
