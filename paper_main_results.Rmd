...
merge.df <- full_join(no.ext_mean.diffs.long, mean.diffs.long)
...
Significance testing (w/in sub. mean abs centile error) 
...
Keeping these analyses in case I want to use one of the above violin/boxplots that show w/in subject averages (across features). However, while these results I think are good to have from a "be aware of how biological variables are distributed across your study sites" perspective, it doesnt really say anything interesting from a "combat methods" perspective.
...
skewed but ~ variances and not paired, using wilcox (mann-whitney u) test. Also calculating the median z-score errors of males and females

```{r, warning=FALSE}
sex_t_tests_in_feat.df <- sex.bias.feat.wilcox.tests(diffs.df)
