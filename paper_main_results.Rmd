...
geom_signif(y_position = c(29, 30.5, 4), xmin = c(0.7, 0.7, 1), 
  ...
  ggtitle("Magnitude error in centile scores")
...
p_diff_annotate <- annotate_figure(p_diff, top = text_grob("Magnitude Centile Errors", color = "black", hjust = 0, x = 0.06))
p_diff_annotate
```
...
```{r}
# custom_strip <- strip_themed(
#      text_x = elem_list_text(angle = 90, color="black"),
...
*need to add stats tests*
```{r}
regional_violin_plt <- plt.df %>%
  dplyr::filter(!pheno %in% vol_list_global) %>%
  ggplot() +
  geom_violin(aes(x=pheno, y=diff, fill=sex, color=sex)) +
  theme_linedraw(base_size = 11) +
  labs(y="Centile Error", x = "Brain Feature") +
  theme(plot.title = element_text(size=12), axis.ticks.x = element_blank(), axis.text.x = element_blank(), panel.spacing.x = unit(.1, "lines"), legend.position="bottom") +
  facet_grid2(dataset ~ pheno_cat, labeller=labeller(dataset=dataset_names), scales="free_x", space = "free_x") +
  ggtitle("Errors in centile scores of global features in ComBat-GAM and ComBatLS")
```


```{r}
global_violin_sex_plt <- diffs.df %>%
  dplyr::select(participant, sex, age_days, sim.site, dataset, paste0("diff_", vol_list_global)) %>%
  dplyr::filter(dataset == "cf.gam" | dataset == "cf.gamlss") %>% #drop refA levels | dataset == "cf.lm"
  pivot_longer(cols=starts_with("diff_"), names_to="pheno", names_prefix="diff_", values_to = "diff") %>%
  mutate(dataset = factor(dataset, levels = c("ukb_CN", "cf", "cf.lm", "cf.gam", "cf.gamlss"), ordered = TRUE),
         diff = diff*100) %>%
  ggplot(aes(x=pheno, y=diff, fill=sex)) +
  geom_rain(likert= TRUE,
            point.args=list(color=NA, fill=NA),
 boxplot.args.pos = list(position = ggpp::position_dodgenudge(x = -.07), width = .3),
 violin.args = list(color=NA, alpha = .7))+
  ...
  labs(y="Centile Error", x = "Brain Feature") +
  facet_wrap(~dataset)
global_violin_sex_plt
```

```{r}
diffs.df %>%
  dplyr::select(participant, sex, age_days, sim.site, dataset, paste0("diff_", pheno_list)) %>%
  dplyr::filter(dataset == "cf.gam" | dataset == "cf.gamlss") %>% #drop refA levels
  pivot_longer(cols=starts_with("diff_"), names_to="pheno", names_prefix="diff_", values_to = "diff") %>%
  mutate(dataset = factor(dataset, levels = c("ukb_CN", "cf", "cf.lm", "cf.gam", "cf.gamlss"), ordered = TRUE),
         diff = diff*100,
    ...
    pheno %in% sa_list ~ "Regional SA",
    pheno %in% ct_list ~ "Regional CT",
    TRUE ~ NA_character_))) %>% #for visuals
  ggplot(aes(y=diff)) +
  geom_violin(aes(x=interaction(pheno,pheno_cat, sep = "&"), y=diff, fill=sex, color=sex)) +
  theme_linedraw(base_size = 11) +
  # scale_fill_manual(values=c(cf.gam = "#3D75C2", cf.lm = "#75C23D", cf.gamlss ="#CFB023FF"), name = "ComBat \nConfiguration", labels=dataset_names) +
  ...
  facet_wrap(~dataset, labeller=labeller(dataset=dataset_names), nrow=2) +
  guides(x = ggh4x::guide_axis_nested(delim = "&")) +
  theme(plot.title = element_text(size=12), axis.ticks.x = element_blank(), axis.text.x = element_blank(), ggh4x.axis.nesttext.x = element_text(angle = 45)) +
  ggtitle("Errors in centile scores of global features in ComBat-GAM and ComBatLS")
```
...
sex.var.plot <- ggplot(sex.var.plot.df) +
  ...
  facet_grid(dataset ~ pheno_cat, labeller = labeller(dataset = as_labeller(dataset_names))) + 
  theme(axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_rect(color= "gray", fill = NA), aspect.ratio=2/3, plot.margin = margin(1,1,1,1, 'pt')) +
  paletteer::scale_fill_paletteer_c("grDevices::Cork", name = "Sex Effect", limits = c(-6, 6)) #
sex.var.plot
```
...
sex.var.plot.global <- ggplot(sex.var.plot.global.df) +
  ...
  theme_linedraw(base_size=12) +
  theme(axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_rect(color= "gray", fill = NA), aspect.ratio=, plot.margin = margin(1,1,1,1, 'pt'), strip.text.y = element_blank()) +
  paletteer::scale_fill_paletteer_c("grDevices::Cork",  limits = c(-6, 6), name = "Sex Effect") #,
# sex.var.plot.global
```
...
wmv_centile_fan <- ggplot(wmv.df) +
  ...
  geom_label(data=tmp_labels, aes(x=age_years, y=value, label=as.character(centile)), nudge_x = 0.35, size=2) +
  xlab("Age (yrs)") + ylab("Volume (mm3)") +
  theme_linedraw(base_size = 11) +
  theme(plot.title = element_text(size=12), legend.position="bottom") +
  labs(title="White matter volume brain charts")
wmv_centile_fan
```
toy brain chart for figs
...
fig_1 <- ggarrange(
  wmv_centile_fan,
  ggarrange(global_cent_errs_plot, p_diff_annotate, ncol=1, nrow=2, heights = c(1.2, 2), common.legend = FALSE, labels=c("B", "C")),
  ncol=2, nrow=1, 
  labels=c("A", NA),
  widths=c(1.1, 1)
  #hjust=0.01,
...
fig_1_annotate
ggsave("figs/figure_1.jpeg", fig_1_annotate, width=11, height=7, units="in")
```
...
fig_2_annotate
ggsave("figs/figure_2.jpeg", fig_2_annotate)
```

```{r}
fig_2.opt2 <- ggarrange(
  p_sex, regional_violin_plt,
  ncol=2, nrow=1, 
  widths = c(1, 1.5),
  #labels=c("A", "B"),
  #hjust=0.01,
...
fig_2.opt2_annotate <- annotate_figure(fig_2.opt2, top = text_grob("Figure 2. Sex-biases in centile score errors across ComBat configurations", color = "black", hjust = 0, x = 0))
fig_2.opt2_annotate
ggsave("figs/figure_2_option2.jpeg", fig_2.opt2_annotate, width=7, height=4, units="in")
