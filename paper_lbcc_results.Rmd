...
# #get w/in cat stats
# delta_summary.cat.df <- cent_deltas_df_long %>%
#   mutate(abs.delta = abs(delta),
#          pheno_cat = case_when(
...
#             site_n=n())
# 
# delta_summary.df <- full_join(delta_summary.df, delta_summary.cat.df)
...
summary(lm(mean_gam_cent ~ site_mean_age, data=gam_centile_site.mean.df2))
summary(lm(mean_gam_cent ~ site_mean_age + sex_prop_Female + site_n, data=gam_centile_site.mean.df2))
```
...
summary(lm(mean_gamlss_cent ~ site_mean_age, data=gamlss_centile_site.mean.df2))
summary(lm(mean_gamlss_cent ~ site_mean_age + sex_prop_Female + site_n, data=gamlss_centile_site.mean.df2))
```

NS

Site-level mean centiles are not associated with study age in either ComBat-GAM or ComBatLS
...
full.delta.age.fit <- lm(site_mean_abs.delta ~ site_mean_age_yrs + site_n + sex_prop_Female, data=delta_sum.df_merged) #scaling to centile space

full.delta.age.fit2 <- lm(site_mean_delta ~ site_mean_age_yrs + site_n + sex_prop_Female, data=delta_sum.df_merged) #scaling to centile space
summary(full.delta.age.fit2)
```
...
#Exploratory: Subject-Level Mean Results

Get subject-level average deltas

```{r}
subj.avg.delta.df <- cent_deltas_df_long %>%
  mutate(abs.delta = abs(delta)) %>%
  group_by(participant) %>%
  summarise(subj_mean_abs.delta=mean(abs.delta),
            subj_mean_delta=mean(delta),
            site=unique(site),
            study=unique(study),
            sex=unique(sex))

#add back some site-level info
subj.avg.delta.sum.df <- left_join(subj.avg.delta.df, delta_sum.df_merged)

subj.avg.delta.sum.df %>%
  ggplot() +
  geom_boxplot(aes(x=sex, y=subj_mean_delta))
```

##Age

ComBat-GAM centile ~ site age

```{r}
#add back some site-level info
gam_centile_df.mean2 <- left_join(gam_centile_df.mean, delta_sum.df_merged)

summary(lm(mean_cent ~ site_mean_age, data=gam_centile_df.mean2))
```

ComBatLS centile ~ site age

```{r}
#add back some site-level info
gamlss_centile_df.mean2 <- left_join(gamlss_centile_df.mean, delta_sum.df_merged)

summary(lm(mean_cent ~ site_mean_age, data=gamlss_centile_df.mean2))
```

Neither combat method produces centiles that are assoc. with site mean age
 
delta ~ age

```{r}
# age
summary(lm(subj_mean_abs.delta ~ site_mean_age, data=subj.avg.delta.sum.df))
summary(lm(subj_mean_delta ~ site_mean_age, data=subj.avg.delta.sum.df))

ggplot(subj.avg.delta.sum.df, aes(x=site_mean_age_yrs, y=subj_mean_abs.delta)) +
  geom_point() +
  geom_smooth(method="lm")
ggplot(subj.avg.delta.sum.df, aes(x=site_mean_age_yrs, y=subj_mean_delta)) +
  geom_point() +
  geom_smooth(method="lm")
```

the difference between combat and combat-ls centiles is

```{r}
#forward selection for subj_mean_abs.delta
summary(lm(subj_mean_abs.delta ~ site_mean_age_yrs + site_n, data=subj.avg.delta.sum.df))
summary(lm(subj_mean_abs.delta ~ site_mean_age_yrs + site_n + sex_prop_Female, data=subj.avg.delta.sum.df)) #small effect but remains significant/grows stronger

#forward selection for subj_mean_abs.delta
summary(lm(subj_mean_delta ~ site_mean_age_yrs + site_n, data=subj.avg.delta.sum.df))
summary(lm(subj_mean_delta ~ site_mean_age_yrs + site_n + sex_prop_Female, data=subj.avg.delta.sum.df))
```
...
# %female
summary(lm(subj_mean_abs.delta ~ sex_prop_Female, data=subj.avg.delta.sum.df))
summary(lm(subj_mean_delta ~ sex_prop_Female, data=subj.avg.delta.sum.df))
summary(lm(subj_mean_abs.delta ~ sex_prop_Female + site_n, data=subj.avg.delta.sum.df)) #add site size covariate
```
...
ggarrange(sample_boxplot, sample_barplot,
          ...
          heights=c(14, 1)),
# mean effects
ggarrange(marg.age_effects.plt,
marg.sex_effects.plt,
nrow=2, ncol=1,
align="hv",
labels=c("B", "C")),
common.legend=FALSE,
...
# mean effects
ggarrange(
ggarrange(age_effects.plt, age_effects.plt.null,
          nrow=2, ncol=1, common.legend=TRUE, legend="bottom",
          align = "hv"),
ggarrange(sex_effects.plt.null, sex_effects.plt,
          nrow=2, ncol=1, common.legend=TRUE, legend="bottom",
          align = "hv"),
# align="hv",
# labels=c("B", "C")),
...
fig5.optB_annotate <- annotate_figure(fig5.optB, top = text_grob("Figure 5. Differences in centile scores of lifespan consortium data when harmonized with ComBat-GAM and ComBatLS", color = "black", hjust = 0, x = 0))
fig5.optB_annotate
ggsave("figs/figure_5_option2.jpeg", fig5.optB_annotate, width=13, height=10, units="in")
```
