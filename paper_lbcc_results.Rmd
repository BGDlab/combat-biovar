...
```{r}
all.pred.df <- reduce(list(gmv.cf.gam.pred.df_full, gmv.cf.gamlss.pred.df_full, gmv.raw.pred.site.df, gmv.raw.pred.df), full_join)
...
```{r}
wmv.all.pred.df <- reduce(list(wmv.raw.pred.site.df, wmv.raw.pred.df, wmv.cf.gam.pred.site.df, wmv.cf.gam.pred.df, wmv.cf.gamlss.pred.site.df, wmv.cf.gamlss.pred.df), full_join)
...
```{r}
sgmv.all.pred.df <- reduce(list(sgmv.raw.pred.site.df, sgmv.raw.pred.df, sgmv.cf.gam.pred.site.df, sgmv.cf.gam.pred.df, sgmv.cf.gamlss.pred.site.df, sgmv.cf.gamlss.pred.df), full_join)
...
# ven.raw.mod.site <- readRDS(paste0(der_path, "/Ventricles_lifespan-CN-imp-sites-euler_raw_site.est_mod.rds"))
# ven.raw.pred.site.df <- pred.across.ages.lbcc.study(ven.raw.mod.site, raw.data)

# ven.raw.pred.site.df <- ven.raw.pred.site.df %>%
#   mutate(study_incl = TRUE,
#          combat= "raw")
...
```{r}
ven.all.pred.df <- reduce(list(ven.raw.pred.df, ven.cf.gam.pred.site.df, ven.cf.gam.pred.df, ven.cf.gamlss.pred.site.df, ven.cf.gamlss.pred.df), full_join) #ven.raw.pred.site.df
