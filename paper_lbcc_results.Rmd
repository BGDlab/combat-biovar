...
feat_aov.df <- full_join(feat_aov.df, df_raw_feat_aov.study)
...
mean_cents.df <- full_join(mean_cents.df, raw_centile_df.mean)
...
#Study-Level Mean Centiles ~ Study Ages

##Unharmonzied

```{r}
raw_centile_df.clean_long <- raw_centile_df.clean %>%
  pivot_longer(cols=any_of(pheno_list), names_to="pheno", values_to = "centile")

#overall means
raw_centile_site.mean.df <- raw_centile_df.clean_long %>%
  group_by(study) %>%
  summarize(mean_raw_cent=mean(centile),
            median_raw_cent=median(centile),
            mean_age=mean(age_years),
            study_n= n()) %>%
  mutate(pheno_cat = "all")

summary(lm(mean_raw_cent ~ mean_age, data=raw_centile_site.mean.df))
summary(lm(mean_raw_cent ~ mean_age + study_n, data=raw_centile_site.mean.df))
```
...
summary(lm(mean_gam_cent ~ mean_age, data=gam_centile_site.mean.df))
summary(lm(median_gam_cent ~ med_age, data=gam_centile_site.mean.df))
summary(lm(mean_gam_cent ~ mean_age + study_n, data=gam_centile_site.mean.df))
```
...
summary(lm(mean_gamlss_cent ~ mean_age, data=gamlss_centile_site.mean.df))
summary(lm(median_gamlss_cent ~ med_age, data=gamlss_centile_site.mean.df))
summary(lm(mean_gamlss_cent ~ mean_age + study_n, data=gamlss_centile_site.mean.df))
```

plots

```{r}
all_means.df <- full_join(gam_centile_site.mean.df, gamlss_centile_site.mean.df)
all_means.df <- full_join(all_means.df, raw_centile_site.mean.df)

all_means.df.long <- all_means.df %>%
  pivot_longer(cols=c("mean_gam_cent", "mean_gamlss_cent", "mean_raw_cent"), names_to="combat", values_to = "mean_cent") %>%
  mutate(combat=gsub("mean_(.+)_cent", "\\1", combat))

all_means.df.long %>%
  ggplot(aes(x=mean_age, y=mean_cent, color=combat)) +
  geom_point(aes(size=study_n)) +
  geom_smooth(method="lm")

all_means.df.long %>%
  ggplot(aes(x=med_age, y=mean_cent, color=combat)) +
  geom_point(aes(size=study_n), alpha=0.6) +
  geom_smooth(method="lm")
```

```{r}
all_medians.df.long <- all_means.df %>%
  pivot_longer(cols=c("median_gam_cent", "median_gamlss_cent", "median_raw_cent"), names_to="combat", values_to = "median_cent") %>%
  mutate(combat=gsub("median_(.+)_cent", "\\1", combat))

all_medians.df.long %>%
  ggplot(aes(x=med_age, y=median_cent, color=combat)) +
  geom_point(aes(size=study_n), alpha=0.6) +
  geom_smooth(method="lm")
```
...
```{r}
study_info <- gam_centile_site.mean.df %>%
  dplyr::select(study, mean_age, med_age, study_n)

raw_centile_df.mean2 <- full_join(raw_centile_df.mean, study_info)
summary(lm(mean_cent ~ mean_age, data=raw_centile_df.mean2))

raw_centile_df.mean2 %>%
  ggplot(aes(x=mean_age, y=mean_cent)) +
  geom_point(aes(color=study), alpha=0.7) +
  geom_smooth(method="lm") +
  theme(legend.position="none")
```
...
summary(lm(mean_cent ~ mean_age, data=gam_centile_df.mean2))
...
summary(lm(mean_cent ~ mean_age, data=gamlss_centile_df.mean2))
...
all_subj_means.df <- full_join(gamlss_centile_df.mean2, gam_centile_df.mean2)
all_subj_means.df <- full_join(all_subj_means.df, raw_centile_df.mean2)
