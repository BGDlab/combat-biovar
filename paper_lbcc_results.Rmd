...
age_effects.plt <- delta_sum.df_piv %>%
  ...
  geom_smooth(method="lm", color="#060B14FF") +
  theme_linedraw(base_size = 11) +
...
sex_effects.plt <- delta_sum.df_piv %>%
  ...
  geom_smooth(method="lm", color="#006027FF") +
  paletteer::scale_color_paletteer_c("grDevices::Greens 2", name = "Mean Age \n(Years)") +
  theme_linedraw(base_size = 11) +
...
fig4_annotate <- annotate_figure(fig4, top = text_grob("Figure 4. Differences in centile scores of lifespan consortium data when harmonized with ComBat-GAM and ComBatLS", color = "black", hjust = 0, x = 0))
fig4_annotate
ggsave("figs/figure_4.jpeg", fig4_annotate, width=12, height=10, units="in")
```
