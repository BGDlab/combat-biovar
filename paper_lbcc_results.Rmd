...
table(df_raw_feat_aov.study$sig_fdr)
```
```{r}
...
ANOVA of subjects' mean centiles across studies
...
table(df_raw_feat_aov.z.study$sig_fdr)
```
...
table(df_gam_feat_aov.study$sig_fdr)
```
...
table(df_gam_feat_aov.z.study$sig_fdr)
```
...
table(df_gamlss_feat_aov.study$sig_fdr)
```
...
table(df_gamlss_feat_aov.z.study$sig_fdr)
```
...
fsq.df <- ggplot(f2.df) +
  ...
  labs(x = "Feature", y = "Effect Size (Cohen's F-Squared)", fill="ComBat")

ggsave("figs/supp_fsq.jpeg", fsq.df, width=8, height=6, units="in")
```
...
Get study info
```{r}
nrow(raw_centile_df.clean) == n_unique(raw_centile_df.clean$participant)
...
See if a subject's distance from their study's mean age is assoc with how much their centile changes
...
Make sure that results of mean centile delta ~ mean study age are related to the difference itself, not because ComBatLS and/or ComBat-GAM centiles are associated with mean study age.
...
Reformatting plots for prettier composite fig
...
mean_delta_fig <- mean_cent_deltas_df_study %>%
  ...
  theme_linedraw() +
  theme(legend.position="none") +
  labs(x="Mean Age of Primary Study Sample (Years)", y="Mean Absolute Difference in Centile Scores (Centiles)", title="Subject-wise Mean Distance Between Centiles Calculated From ComBatLS and ComBat-GAM \n-harmonized Data")
ggsave("figs/supp_mean_delta_fig.jpeg", mean_delta_fig, width=9, height=6, units="in")
```

lines show marginal effects of mean study age
...
mean_delta_fig2 <- mean_cent_deltas_df_study %>%
  ...
  theme_linedraw() +
  theme(legend.position="none") +
  labs(x="Subject's Offset from Mean Age of Primary Study (Years)", y="Mean Absolute Difference in Centile Scores (Centiles)", title="Subject-wise Mean Distance Between Centiles Calculated From ComBatLS and ComBat-GAM \n-harmonized Data")
ggsave("figs/supp_mean_delta_fig2.jpeg", mean_delta_fig2, width=9, height=6, units="in")
```
