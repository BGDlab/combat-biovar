...
## Simulate Sites
Dividing each subject into 3 sites (A, B, and C) with M:F ratios of 1:1, 4:1, and 1:4
...
ukb_df_sites %>%
  group_by(sim.site) %>%
  summarise(min = min(age_days), max = max(age_days), mean = mean(age_days))
```

Center and visualize distributions

```{r}
ukb_sites_centered <- ukb_df_sites %>%
  group_by(sex) %>%
...
global_site_plots <- plot.hist.by.sex(ukb_sites_centered, vol_list_global, facet_fac = "sim.site")
