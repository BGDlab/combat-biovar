...
cent.df <- df %>%
  dplyr::select(participant, sex, age_days, sim.site, log_age, sexMale, sex.age, fs_version, TBV, Vol_total, SA_total, CT_total)
...
var.list <- lapply(model.files, get.var.across.ages.lbcc, og_df = df)
