...
#csv name = fname_str w/o _cb_mod
fname_str_csv <- gsub("_cb_mod", "", fname_str)
...
cent.list <- lapply(model.files, pred_og_centile, og.data = df, get.std.scores = TRUE)
...
cent.df <- df %>%
  dplyr::select(participant, sex, age_days, study, log_age, sexMale, sex.age, fs_version)
