...
gamlss_fit <- function(pheno, fam= "BCCG"){
  result <- tryCatch({
    gamlss_RSformula <-paste("gamlss(formula =", pheno, "~ pb(age_days) + sexMale + fs_version + pb(sex.age),",
                             "sigma.formula = ~ pb(age_days) + sexMale + fs_version + pb(sex.age),",
                             "nu.formula = ~ pb(age_days) + sexMale + fs_version + pb(sex.age),", 
    ...
    tryCatch({
      gamlss_CGformula <- paste("gamlss(formula =", pheno, "~ pb(age_days) + sexMale + fs_version + pb(sex.age),",
                                "sigma.formula = ~ pb(age_days) + sexMale + fs_version + pb(sex.age),",
                                "nu.formula = ~ pb(age_days) + sexMale + fs_version + pb(sex.age),", 
...
if(grepl("raw", csv_name, fixed=TRUE)){
  ...
  base_model <- gamlss_fit(pheno, "BCCGo")
} else {
  base_model <- gamlss_fit(pheno, "BCCG")
}
