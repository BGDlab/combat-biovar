...
var.plot <- ggplot(var.plot.df) +
  geom_rect(data = tp, fill="lightgreen", xmin = -Inf,xmax = Inf,
...
```{r}
gridExtra::grid.arrange(var.plot.global,var.plot, ncol=2, widths=c(1, 1.7))
```


```{r}
p2 <- grid_arrange_shared_legend(var.plot.global, var.plot, ncol=2, nrow=1, widths=c(1, 1.7))
annotate_figure(p2, top = text_grob("Standardized effect of male sex on feature variance", 
               color = "black", size = 16))
```
