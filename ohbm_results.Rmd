...
```{r}
permutation_path <- "/Users/megardn/Desktop/BGD_Repos/combat_biovar/ukb_perm_csvs"
```
...
```{r}
fwrite(z_t.tests.p, file="ukb_permute/z_t.tests.csv")
```
...
```{r}
plot_male_var <- function(df) {
permutation <- unique(df[["perm"]])

sex_effect_plot <- df %>%
  mutate(sex_standard = case_when(isTRUE(sig.drop_bf.corr)~ sex_standard,
                                   isFALSE(sig.drop_bf.corr) ~ NA)) %>% #only significant effects
  ggplot() +
  geom_bar(aes(x=pheno, y=sex_standard, fill=dataset)) +
  facet_wrap(~pheno_cat) +
  ggtitle(paste("Standardized effect of male sex on variance:", permutation))

  ggsave(paste0("figs/male_variance_ohbm", permutation, ".jpeg", sex_effect_plot)
